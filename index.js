...
const stream_message = `851585780023427114`;
        ...
        case stream_message:
            if (reaction.emoji.name == `🐼`) {
                reaction.message.guild.members.fetch(user)
                    .then((member) => {
                        member.roles.remove(`850041535818891307`).catch(console.error);
                    });
            }
            break;
        case rules_message:
    ...
    switch (reaction.message.id) {
        ...
        case stream_message:
            if (reaction.emoji.name == `🐼`) {
                reaction.message.guild.members.fetch(user)
                    .then((member) => {
                        member.roles.add(`850041535818891307`).catch(console.error);
                    });
            }
            break;
    }
